    ...
    for type in ['yaml', 'coffee', 'js']
      do (type) ->
        describe "with #{type} config file", ->
          it 'supports arrays', ->
            config = new Config("#{__dirname}/configs/arrays.#{type}", 'foo')
          ...
          it 'replaces array value by default', ->
            config = new Config("#{__dirname}/configs/arrays.#{type}", 'bar')
          ...
          it 'replaces array value by default 2', ->
            config = new Config("#{__dirname}/configs/arrays.#{type}", 'derp')
